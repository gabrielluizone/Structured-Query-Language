...
FROM
   ...
   */
  REM Script: HR Objects
  and Data For Live SQL REM This script will create a the HR Sample Schema objects
  and data in your local schema.If you want just query - only,
  you can instead use the HR sample schema by referencing hr.regions,
  etc.To drop the objects once created,
  you can run "Drop HR Sample Schema" - https: / / livesql.oracle.com / apex / livesql / file / content_GWKN7QJBHHC8F1RJTEB47AFOY.html.Please note that this schema was initially created in 2000
  and last updated March 19,
  2015 so the constructs are not necessarily what we would recommend today.-- Copyright
...
CREATE TABLE
  locations (
    location_id NUMBER(4) NOT NULL CONSTRAINT loc_id_pk PRIMARY KEY,
...
-- Starts with 3300 
CREATE SEQUENCE locations_seq START
WITH
  3300 INCREMENT BY 100 MAXVALUE 9900 NOCACHE NOCYCLE;
...
-- Starts with 280 
CREATE SEQUENCE departments_seq START
WITH
  280 INCREMENT BY 10 MAXVALUE 9990 NOCACHE NOCYCLE;
...
-- Starts with 207  
CREATE SEQUENCE employees_seq START
WITH 207 INCREMENT BY 1 NOCACHE NOCYCLE;
...
ALTER TABLE
  departments DISABLE CONSTRAINT dept_mgr_fk;
...
ALTER TABLE
  departments ENABLE CONSTRAINT dept_mgr_fk;

-- procedure to limit edit of EMPLOYEES to normal office hours
CREATE
OR REPLACE PROCEDURE secure_dml IS BEGIN IF TO_CHAR (SYSDATE, 'HH24:MI') NOT BETWEEN '08:00' AND '18:00'
OR TO_CHAR (SYSDATE, 'DY') IN ('SAT', 'SUN') THEN RAISE_APPLICATION_ERROR (
  -20205,
...
CREATE
OR REPLACE TRIGGER secure_employees BEFORE INSERT
OR
UPDATE
  OR DELETE ON employees BEGIN secure_dml;
...
CREATE
OR REPLACE TRIGGER update_job_history AFTER
UPDATE
  OF job_id,
  department_id ON employees FOR EACH ROW BEGIN add_job_history(
: old.employee_id,
: old.hire_date,
    sysdate,
: old.job_id,
...
COMMENT ON TABLE locations IS 'Locations table that contains specific address of a specific office, warehouse, and/or production site of a company. Does not store addresses or locations of customers. Contains 23 rows; references with the departments and countries tables. ';
